...
/**
 * Configuración de diferentes tipos de ejecución de transacciones:
 * - Direct: El merchant paga el gas directamente
 * - Relayer: Meta-transacciones via relayer (EIP-2771)
 * - AccountAbstraction: ERC-4337 con Paymaster
 */
enum class ExecutionMode {
    DIRECT,          // Merchant paga gas directamente
    RELAYER,         // Meta-tx via relayer (Gelato, OpenZeppelin Defender, etc)
    ACCOUNT_ABSTRACTION  // ERC-4337 con Paymaster
}
...
data class PaymasterConfig(
    ...
    val entryPointAddress: String = "0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789", // v0.6
    val chainIds: List<Long>,
    val requiresApiKey: Boolean = true
) {
    companion object {
        val PIMLICO = PaymasterConfig(
            name = "Pimlico",
...
data class PublicBundler(
    val name: String,
    val urlTemplate: String,  // Use {chainId} as placeholder
    val chainIds: List<Long>,
    val requiresApiKey: Boolean = false,
    val rateLimited: Boolean = true,
    val supportsPaymaster: Boolean = false
) {
    fun getUrl(chainId: Long, apiKey: String? = null): String {
    ...
    companion object {
        ...
        val PUBLIC_BUNDLERS = listOf(
            // Pimlico - Free tier for testnets, rate-limited mainnet
            PublicBundler(
                name = "Pimlico Public",
                urlTemplate = "https://api.pimlico.io/v2/{chainId}/rpc",
                chainIds = listOf(84532, 11155111, 421614, 11155420), // Base Sepolia, Sepolia, Arb Sepolia, OP Sepolia
            ...
            PublicBundler(
                ...
                chainIds = listOf(421614), // Arb Sepolia
                requiresApiKey = true,
                rateLimited = true,
            ...
            PublicBundler(
                ...
                chainIds = listOf(84532, 11155111), // Testnets
                requiresApiKey = false,
                rateLimited = true,
                supportsPaymaster = false
            ),
            ...
            PublicBundler(
                ...
                chainIds = listOf(11155111), // Sepolia
                requiresApiKey = false,
                rateLimited = true,
                supportsPaymaster = false
            )
...
object BundlerSelector {
    ...
    fun selectBundler(
        ...
        val publicBundler = PublicBundler.getBundlersForChain(chainId)
            .firstOrNull { !it.requiresApiKey }
        ...
        if (!apiKey.isNullOrBlank()) {
            val provider = listOf(PaymasterConfig.PIMLICO, PaymasterConfig.STACKUP, PaymasterConfig.ALCHEMY)
...
// Request genérico para meta-transacciones (EIP-2771)
